	...
	<body>
		...
		<div id="show-album" class="content scaffold-show" role="main">
			...
			<ol class="property-list album">
				...
				<g:if test="${albumInstance?.author}">
				<li class="fieldcontain">
					<span id="author-label" class="property-label"><g:message code="album.author.label" default="Author" /></span>
					
						<span class="property-value" aria-labelledby="author-label"><g:link controller="author" action="show" id="${albumInstance?.author?.id}">${albumInstance?.author?.encodeAsHTML()}</g:link></span>
